...
class HrEmployee(models.Model):
    ...
    # all_document_employee from hr_employee_document
        
    all_document_employee = fields.Many2many('hr.employee.document', compute="get_all_employee_document",
                                                string="All Documents")
    def get_all_employee_document(self):
        for each in self:
            document_ids = self.env['hr.employee.document'].sudo().search([('employee_ref', '=', each.id)])
            each.all_document_employee = document_ids

    have_document = fields.Boolean(compute="check_document", string="Have Document")
    def check_document(self):
        for each in self:
            document_ids = self.env['hr.employee.document'].sudo().search([('employee_ref', '=', each.id)])
            if document_ids:
                each.have_document = True
            else:
                each.have_document = False                                
