...
class AccountMoveLine(models.Model):
    ...
    def _query_get(self, domain=None):
        ...
        if self._context.get("operating_unit_ids", False):
            ou_ids = self._context.get("operating_unit_ids")
            if type(ou_ids) != list:
                ou_ids = self._context.get("operating_unit_ids").ids
            domain.append(
                ("operating_unit_id", "in", ou_ids)
                # ("operating_unit_id", "in", self._context.get("operating_unit_ids").ids)
            )
