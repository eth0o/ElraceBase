                                ...
                                if uid.user_id == each.user_id:
                                    ...
                                    if get_user_id:
                                        ...
                                        if duplicate_atten_ids:
                                            ...
                                            print("attendance work on this",zk_attendance)
                                            att_var = att_obj.search([('employee_id', '=', get_user_id.id),
                                                                      ('check_out', '=', False),('date','=',atten_time)])
                                          
                                            if att_var:
                                                print('ddfcd', str(each.status))
                                                print(str(att_var.employee_id.name))
                                                print(str(att_var.check_in))
                                                print(str(att_var.date))
                                                print("atten_time",str(atten_time))
                                                # att_var.filtered(lambda l: l.date == atten_time)
                                                # print("*************after****",str(att_var.check_in))
                                            if each.punch == 0:  # check-in
                                                # if att_var:
                                                #     raise UserError(str(att_var))
                                                if not att_var:
                                                    att_obj.create({'employee_id': get_user_id.id,
                                                                    'check_in': atten_time})
                                            if each.punch == 1:  # check-out
                                                if len(att_var) == 1:
                                                    print("att_var:  # check-out",atten_time)
                                                    att_var.write({'check_out': atten_time})
                                                else:
                                                    att_var1 = att_obj.search([('employee_id', '=', get_user_id.id),('date','=',atten_time)])
                                                    if att_var1:
                                                        print("att_var1",att_var1.employee_id.name)
                                                        print("att_var1 checkin",att_var1[-1].check_in)
                                                        print("att_var1 checkout",att_var1[-1].check_out)
                                                        
                                                        att_var1[-1].write({'check_out': atten_time})
                                                        print("After att_var1 checkout ",att_var1[-1].check_out)
                                                        # raise UserError(att_var1[-1].check_in)
                                                    # else:
                                                        
                                                    #     print ("Atten time", atten_time)
                                                    #     print("User is ",get_user_id.name)
                                                    #     att_obj.create({'employee_id': get_user_id.id,
                                                    #                 'check_out': atten_time ,'check_in':False})
                                                        # raise UserError("Here")
                                            print("*******************\n\n\n att_obj*************",str(att_obj))

                                    else:
                                        print('ELSE', str(each.status))
                                        print('user', uid.name)
